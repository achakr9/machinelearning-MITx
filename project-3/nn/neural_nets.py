...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        ouLA_vec = np.vectorize(output_layer_activation)
        activated_output = output_layer_activation(output) # a_hat TODO

        ### Backpropagation ###

        # Compute gradients
        output_layer_error = -(y-activated_output) # derivative of C TODO

        ouLA_deriv_vec = np.vectorize(output_layer_activation_derivative)
        output_deriv_activ = ouLA_deriv_vec(activated_output)
        hidden_layer_error = np.multiply(output_deriv_activ, self.hidden_to_output_weights.transpose()) * output_layer_error # TODO (3 by 1 matrix)

        reLU_deriv_vec = np.vectorize(rectified_linear_unit_derivative)
        bias_gradients = np.multiply(hidden_layer_error, reLU_deriv_vec(hidden_layer_weighted_input)) # TODO
        hidden_to_output_weight_gradients = np.multiply(hidden_layer_activation, output_layer_error).transpose() # TODO
        input_to_hidden_weight_gradients = np.dot(bias_gradients, input_values.transpose())# TODO
